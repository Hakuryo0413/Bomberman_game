...
public class Bom extends BaseObject {
    ...
    public Bom(float x, float y, int length, GameScreen gameScreen) {
        ...
        animation = new Animation(Duration.millis(600), "assets/bom", 4);
        animation.play();
        Timeline tl = new Timeline(new KeyFrame(Duration.seconds(2), actionEvent -> {
            ...
            if (maxLength && map[j - length][i] == 0)
                gameScreen.getFireList().add(new Fire(Fire.FireType.TOP, (int) this.x, (int) this.y - length * GlobalConstant.TILE_SIZE, gameScreen));
            maxLength = true;
            for (int t = 1; t < length; t++) {
                if (map[j + t][i] == 0) {
                    gameScreen.getFireList().add(new Fire(Fire.FireType.VERTICAL, (int) this.x, (int) this.y + t * GlobalConstant.TILE_SIZE, gameScreen));
            ...
            if (maxLength && map[j + length][i] == 0)
                gameScreen.getFireList().add(new Fire(Fire.FireType.BOTTOM, (int) this.x, (int) this.y + length * GlobalConstant.TILE_SIZE, gameScreen));
            maxLength = true;
            for (int t = 1; t < length; t++) {
                if (map[j][i - t] == 0) {
            ...
            if (maxLength && map[j][i - length] == 0)
                gameScreen.getFireList().add(new Fire(Fire.FireType.LEFT, (int) this.x - length * GlobalConstant.TILE_SIZE, (int) this.y, gameScreen));
            maxLength = true;
            for (int t = 1; t < length; t++) {
                if (map[j][i + t] == 0) {
                    gameScreen.getFireList().add(new Fire(Fire.FireType.HORIZON, (int) this.x + t * GlobalConstant.TILE_SIZE, (int) this.y, gameScreen));
            ...
            if (maxLength && map[j][i + length] == 0)
                gameScreen.getFireList().add(new Fire(Fire.FireType.RIGHT, (int) this.x + length * GlobalConstant.TILE_SIZE, (int) this.y, gameScreen));
        }));
        tl.setCycleCount(1);
        tl.play();
    }
